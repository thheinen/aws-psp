...
## MODIFIED BY ADDING UID MAPPING ##
...
action_class do
  def load_current_resource
    ...
    begin
      resource_data = rest_get&.data
    rescue TrainPlugins::Rest::BadRequest => e
      if e.message.include? 'does not exist'
        # logger.error "Resource with REST ID #{rest_id} was removed externally"
        remove_rest_id
      end
    end
...
action_class do
  ...
  def rest_post(data)
    ...
    if current_resource.class.rest_id_field
      rest_id = JMESPath.search(current_resource.class.rest_id_field, response)
